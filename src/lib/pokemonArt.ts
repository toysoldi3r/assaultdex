...
// origins". Two styles ship: "artwork" (official 2D artwork) and "sprites" (the
// front battle sprites). A species with no art file falls back to the pixel
// menu icon, so nothing ever renders blank.

/** Pokémon art styles that ship as self-hosted files (the pixel icon sheet is
 *  handled separately by PokeIcon, not here). */
export type PokemonArtStyle = "artwork" | "sprites";
